# VR SURGERY
Surgery robots are fascinating but as they are so expensive! As a result the technology remains limited to a handful of people. "Surgery Robot Simulator" aims to solve this issue by replicating the experience of using a surgery robot (modeled based off the DaVinci System). The program's goal is to ultimately expose the user to basic controls of surgery robots such as grabbing, cutting, and suturing. A point system is in place to help the user perform a certain task with a high amount of percision, like placing a box into a container accurately.
